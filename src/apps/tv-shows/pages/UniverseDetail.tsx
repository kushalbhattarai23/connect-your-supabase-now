    ...
    <div className="container mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6 max-w-7xl">
      ...
      <div className="flex flex-col space-y-4 lg:flex-row lg:items-start lg:justify-between lg:space-y-0 lg:space-x-4">
        <div className="space-y-2 flex-1 min-w-0">
          <div className="flex flex-col space-y-2 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-2">
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-blue-700 truncate">
              {universe.name}
            </h1>
            <Badge 
              variant="outline" 
              className={`${universe.is_public ? "border-green-200 text-green-700" : "border-yellow-200 text-yellow-700"} flex-shrink-0 w-fit`}
            >
              {universe.is_public ? (
                <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
        <Card className="border-blue-200">
          <CardContent className="p-3 sm:p-4 text-center">
            <div className="text-xl sm:text-2xl font-bold text-blue-700">{universeShows.length}</div>
            <p className="text-xs sm:text-sm text-muted-foreground">Total Shows</p>
          </CardContent>
        </Card>
        <Card className="border-blue-200">
          <CardContent className="p-3 sm:p-4 text-center">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">Public Shows</p>
          </CardContent>
        </Card>
        <Card className="border-blue-200 col-span-2 sm:col-span-1">
          <CardContent className="p-3 sm:p-4 text-center">
            <div className="text-xl sm:text-2xl font-bold text-blue-700">
      ...
      <Tabs defaultValue="shows" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="shows" className="text-sm">Shows</TabsTrigger>
          <TabsTrigger value="episodes" className="text-sm">Episodes</TabsTrigger>
        </TabsList>
        
        <TabsContent value="shows" className="space-y-4 mt-4">
          <div>
            <h2 className="text-lg sm:text-xl font-semibold text-blue-700 mb-4">Shows in this Universe</h2>
                  ...
                  <Card key={universeShow.id} className="border-blue-200 hover:shadow-lg transition-shadow">
                    <CardHeader className="pb-3">
                      <div className="flex items-start justify-between gap-2">
                        <CardTitle className="text-blue-700 text-base sm:text-lg line-clamp-2 leading-tight">
        ...
        <TabsContent value="episodes" className="space-y-4 mt-4">
          <div>
            <h2 className="text-lg sm:text-xl font-semibold text-blue-700 mb-4">Episodes in this Universe</h2>
