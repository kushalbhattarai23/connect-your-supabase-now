...
export const Universes: React.FC = () => {
  const { user } = useAuth();
  const { universes, isLoading: isLoadingPrivate, createUniverse, deleteUniverse } = useUniverses();
  const { publicUniverses, isLoading: isLoadingPublic } = usePublicUniverses();
  ...
  const otherPublicUniverses = publicUniverses.filter(universe => universe.creator_id !== user?.id);
  ...
  const UniverseCard = ({ universe, showDelete = false }: { universe: any, showDelete?: boolean }) => (
    <Card 
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-muted-foreground">
          <div className="flex items-center">
            ...
            <span>{universe.creator_id === user?.id ? 'You' : 'Creator'}</span>
          </div>
          <div className="flex items-center">
          ...
          {myUniverses.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {myUniverses.map((universe) => (
                <UniverseCard key={universe.id} universe={universe} showDelete={true} />
              ))}
            </div>
          )}
        </TabsContent>
        
        <TabsContent value="public-universes" className="space-y-6">
          {otherPublicUniverses.length === 0 ? (
            <Card className="border-blue-200">
              <CardContent className="text-center py-12">
                <Globe className="h-16 w-16 text-blue-500 mx-auto mb-4" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {otherPublicUniverses.map((universe) => (
